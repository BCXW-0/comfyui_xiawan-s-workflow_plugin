...
class SaveImagePlus:
    ...
    def check_lazy_status(self, images=None, enable=True, **kwargs):
        if not enable:
            return []
        if images is None:
            return ["images"]
    ...
    def _extract_seed_from_prompt(self, prompt_obj: dict) -> int:
        ...
        def resolve_seed_value(seed_value, visited=None):
            if visited is None:
                visited = set()
            ...
            if not (
                isinstance(seed_value, (list, tuple))
                and len(seed_value) == 2
                and isinstance(actual_prompt, dict)
            ):
                return None

            source_id = str(seed_value[0])
            if source_id in visited:
                return None
            visited.add(source_id)

            source_node = actual_prompt.get(source_id)
            ...
            if class_type == "XiawanGlobalSeedManager":
                operation = inputs.get("operation", "fixed")
                current_seed = resolve_seed_value(inputs.get("current_seed"), visited)
                if current_seed is not None:
                    return current_seed
                seed = resolve_seed_value(inputs.get("seed_value"), visited)
                if seed is not None and operation in ("fixed", "last"):
                    return seed

            for input_name in ("seed", "noise_seed", "seed_value"):
                if input_name in inputs:
                    seed = resolve_seed_value(inputs[input_name], visited)
                    if seed is not None:
                        return seed
            ...
            if isinstance(node_data, dict):
                ...
                if class_type in sampler_types and "inputs" in node_data:
                    ...
                    if "seed" in inputs:
                        ...
                        resolved_seed = resolve_seed_value(seed_value)
                        logger.debug(f"在 sampler 节点 {node_id} ({class_type}) 中找到 seed: {seed_value}, resolved: {resolved_seed}")
                        if resolved_seed is not None:
                            return resolved_seed
                    if "noise_seed" in inputs:
                        seed_value = inputs["noise_seed"]
                        resolved_seed = resolve_seed_value(seed_value)
                        logger.debug(f"在 sampler 节点 {node_id} ({class_type}) 中找到 noise_seed: {seed_value}, resolved: {resolved_seed}")
                        if resolved_seed is not None:
                            return resolved_seed
        ...
        for node_id, node_data in actual_prompt.items():
            if isinstance(node_data, dict) and "inputs" in node_data:
                ...
                if "seed" in inputs:
                    ...
                    resolved_seed = resolve_seed_value(seed_value)
                    logger.debug(f"在节点 {node_id} ({class_type}) 中找到 seed: {seed_value}, resolved: {resolved_seed}")
                    if resolved_seed is not None:
                        return resolved_seed
                if "noise_seed" in inputs:
                    seed_value = inputs["noise_seed"]
                    class_type = node_data.get("class_type", "未知")
                    resolved_seed = resolve_seed_value(seed_value)
                    logger.debug(f"在节点 {node_id} ({class_type}) 中找到 noise_seed: {seed_value}, resolved: {resolved_seed}")
                    if resolved_seed is not None:
                        return resolved_seed
    ...
    def save_images(
        ...
        if images is None or len(images) == 0:
            raise RuntimeError("SaveImagePlus received no images. Check the final output selector and enabled workflow branches.")
