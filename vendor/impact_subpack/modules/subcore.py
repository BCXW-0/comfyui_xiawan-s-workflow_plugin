...
def torch_wrapper(*args, **kwargs):
    ...
    if hasattr(torch.serialization, 'safe_globals'):
        ...
        try:
            ...
            # Check if the error is caused by disallowed globals during the
            # safe load (weights_only=True). In PyTorch >= 2.6 this includes
            # not only the original 'getattr' case but also any unsupported
            # global symbol such as ultralytics.nn.tasks.DetectionModel /
            # SegmentationModel that legacy .pt YOLO checkpoints rely on.
            err_str = str(e)
            is_disallowed_global_error = (
                'getattr' in err_str
                or 'Unsupported global' in err_str
                or 'WeightsUnpickler error' in err_str
                or 'add_safe_globals' in err_str
                or 'safe_globals' in err_str
