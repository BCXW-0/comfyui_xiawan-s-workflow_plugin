import { app as xiawanApp } from "../../scripts/app.js";
...
// 加载CSS修复文件
(function() {
    const possiblePaths = [new URL('./xiawan_weilin_fix.css', import.meta.url).href];
...
if (window.weilinExtensionRegistered) {
  ...
  function waitForApp(callback, maxAttempts = 50) {
    ...
    const check = () => {
      ...
      const app = xiawanApp || window.comfyAPI?.app?.app || window.app || window.comfyApp;
      if (app) {
